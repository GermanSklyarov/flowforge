...
export const defaultNodeHandlers: NodeHandlerRegistry = {
  ...
  'ai.llm': async ({ inboundOutputs, node }) => {
    ...
    return {
      output: {
        response: `${instruction}: ${sourceText ?? 'No input provided.'}`
      }
    };
  },
  ...
  'task.create': async ({ inboundOutputs, node }) => {
    const title = firstString([
      node.config?.title,
      ...Object.values(inboundOutputs).flatMap((output) => [output.title, output.response, output.text])
    ]);
